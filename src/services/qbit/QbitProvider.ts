...
export default class QBitProvider implements IProvider {
  ...
  public setBaseURL(url: string) {
    const targetURL = url || 'api/v2'
    this.axios.defaults.baseURL = targetURL
    if (typeof window !== 'undefined') {
      if (url) {
        localStorage.setItem('auratorrent_active_server_url', url)
      } else {
        localStorage.removeItem('auratorrent_active_server_url')
      }
    }
  }
